...
class GitHistoryWalkerModuleTest: BaseTest() {
    ...
    fun testImport() {
        ...
        for (targetFileLog in targetFileLogs.reversed()) {
            ...
            treeService.importTrees(trees.features.map { it.properties }.map { it.timestamp = targetFileLog.commitTime.toLong().times(1000); it }.filter { it.timestamp != null && it.timestamp!! > 1646842109000 })
